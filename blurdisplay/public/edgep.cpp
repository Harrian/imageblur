...
void blurPixalsAndWrite(std::vector< std::vector<pixal *> * > * &aop,int h,int w,unsigned char * &header,std::string outputFile){
	...
	for(int i=0;i<h;i++){
		newaop->at(i)=new std::vector<pixal *>;
		for(int j=0;j<w;j++){
				int sumOfReds=8*aop->at(i)->at(j)->getRed();
				int sumOfBlues=8*aop->at(i)->at(j)->getBlue();
				int sumOfGreens=8*aop->at(i)->at(j)->getGreen();
				for(int k=-1;k<2;k++)
					for(int l=-1;l<2;l++)
						if(k==0&&l==0)
							continue;
    ...
    auto collection = conn["meteor"]["rightImages"];
...
//			std::cout<<(int)*(readpixal+2)<<" "<<(int)*(readpixal+1)<<" "<<(int)*(readpixal)<<std::endl;
	...
	for(int i=0;i<atoi(argc[2]);i++)
		(i+1)%atoi(argc[3])?blurPixals(bigoldarrayofpixals,imageheight,imagewidth):blurPixalsAndWrite(bigoldarrayofpixals,imageheight,imagewidth,infotable,name+std::to_string(i+1)+"para.bmp");
